...
func (o *order) getOrderItems(db *sql.DB) error {
	rows, err := db.Query("SELECT order_id, product_id, quantity FROM order_items WHERE order_id = ?", o.ID)
	if err != nil {
		return err
	}
	...
	for rows.Next() {
		...
		if err := rows.Scan(&oi.OrderID, &productIDStr, &oi.Quantity); err != nil {
			return err
		}
		oi.ProductID, _ = strconv.Atoi(productIDStr)
